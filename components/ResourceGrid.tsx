      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 mb-10">
          ...
          >
            ...
            <div className="flex items-center gap-1 p-1 rounded-lg glass-card border border-muted">
              <button
                id="view-grid-btn"
                onClick={() => setViewMode('grid')}
                aria-label="Grid view"
                className={`p-1.5 rounded-md transition-all duration-200 ${
                  viewMode === 'grid' ? 'bg-violet-500/20 text-violet-400' : 'text-muted hover:text-muted'
                }`}
              >
                <LayoutGrid size={14} />
              </button>
              <button
                ...
                className={`p-1.5 rounded-md transition-all duration-200 ${
                  viewMode === 'list' ? 'bg-violet-500/20 text-violet-400' : 'text-muted hover:text-muted'
                }`}
              >
                <List size={14} />
              </button>
        ...
        {!loading && (
          <AnimatePresence mode="wait">
            {filtered.length > 0 ? (
              ...
              >
                <div className="w-16 h-16 rounded-2xl glass-card border border-muted flex items-center justify-center">
                  <Filter size={24} className="text-muted" />
                </div>
                <div>
                  <p className="text-muted text-sm font-medium mb-1">No resources found</p>
                  <p className="text-muted text-xs">Try a different search or filter</p>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
